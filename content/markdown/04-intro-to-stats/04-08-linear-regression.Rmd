...
Recall that at the beginning of the previous section, we asked if biomass affected species richness, but <em>also</em> if that was the case, if that effect differed across levels of pH. 

We'll essentially walk through all the steps from section I but with less commentary. 
...
We already did this above, a poisson distribution with a log link function is sufficient for our purposes here. 

#### Step 4 - Fit the Model 

Before we go further, I'll note here that what we asked about was the relationship between Biomass on Species Richness and <em>if pH has an effect on that relationship</em>.

Let's go ahead and fit our model with a crossed interaction, and then take some time to walk through it: 

```{r}
mod_poi_ph <- stats::glm(Species ~ Biomass*pH,
                      data = df,
                      family = poisson(link = log))
...
Let's state this mathematically as I think that's useful to orient ourselves. We'll treat biomass as $X_1 = B$ and pH as $X_2 = P$. Species richness we'll state as $Y = SR$, and if we go back to our equations from above, we're looking for our mean value, so let's state in these terms: 

$$\hat{\text{SR}} = \beta_0  + \beta_1B + \beta_2P + \beta_3BP$$
This is actually what we're asking our model to predict. However, if we think about it, biomass is continuous, but pH is categorical. What does that mean? 
...
Okay, so we see some very very clear groupings here! 